<template>
	<view class="uni-page uni-bg__white">
		<u-sticky style="background-color: white;">
			<u-search customStyle="padding: 8px 16px;" show-action actionText="搜索" v-model="filter.name" animation @custom="getList(1)" @clear="getList(1)" placeholder="请输入建筑名称"></u-search>
		</u-sticky>
		<u-cell-group>
			<u-cell v-for="item in list.data" :title="item.name" :label="item.baseArea" :url="'./detail?id=' + item.id" isLink :key="item.id"></u-cell>
	...
	export default {
		...
		methods: {
			async getList(current = 1) {
				try{
					const res = await uni.$u.http.get("/building/getBuildingList", {
						params: {
							current,
							size: this.list.size,
							...this.filter
